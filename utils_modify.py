...
def train_log(obj, filename='train_log.txt'):
    print(obj)
    if _log_path is not None:
        with open(os.path.join(_log_path, filename), 'a') as f:
            print(obj, file=f)

def val_log(obj, filename='val_log.txt'):
    print(obj)
    if _log_path is not None:
        with open(os.path.join(_log_path, filename), 'a') as f:
            print(obj, file=f)
...
def set_save_path(save_path, remove=True):
    ...
    writer = SummaryWriter(log_dir=os.path.join(save_path, 'train'), comment='train')

    return train_log, val_log, writer
...
def calc_mirror(y_pred, y_true):
    ...
    return mae, np.array(0), np.array(0), np.array(0)
